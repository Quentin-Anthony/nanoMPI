...
int nanompi_init_comm(nanompi_communicator_t **comm_dptr, int rank,
                      int world_size, char *hostfile) {
  ...
  // MPI_COMM_WORLD
  nanompi_comm_world =
      (nanompi_communicator_t *)malloc(sizeof(nanompi_communicator_t));
  ...
  status = nanompi_init_group(&nanompi_comm_world->local_group, rank,
                              world_size, hostfile);
  if (status) {
