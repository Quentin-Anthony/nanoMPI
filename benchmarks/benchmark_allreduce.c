...
double benchmark_allreduce(void *sendbuf, void *recvbuf, int count,
...
int main(int argc, char **argv) {
  ...
  if (rank == 0) {
    PRINT_STDOUT("%-25s %-20s %-20s %-20s\n", "Message Size (bytes)",
                 "Latency (us)", "Bus BW (MB/s)", "Validation");
  }

  for (int message_size = MIN_MESSAGE_SIZE; message_size <= MAX_MESSAGE_SIZE;
       message_size *= 2) {
    int count = message_size / sizeof(int);
    double time = benchmark_allreduce(sendbuf, recvbuf, count, MPI_INT, MPI_SUM,
                                      MPI_COMM_WORLD);

    double busBW =
        ((message_size / 1e6) / (time / 1e6)) *
        (2 * (size - 1) /
         size); // Allreduce bus bandwidth = alg_bw * (2*(n-1)/n), from
