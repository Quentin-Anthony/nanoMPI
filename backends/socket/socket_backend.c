...
static int init_server(nanompi_communicator_t *comm) {
  ...
  // This is especially useful right now because the job launcher does not
  // forward Ctrl+C signals to each process. That means after every run that
  // errors or is Ctrl+C'd out, we get more stray processes
  ...
  // Nagle's algorithm is an OS-level buffering solution. The idea is for
  // chat-like applications where a user is typing in one byte at a time, the
  // socket can coalesce (i.e., merge multiple small sends into one large send)
  // these bytes to massively increase throughput by reducing the amount of
  // control messages going back and forth. We want it off so that for small
  // messages, it doesn't wait for the timeout before sending, messing up the
  // latency measurement.
  if (setsockopt(comm->socket_info.server_fd, IPPROTO_TCP, TCP_NODELAY,
                 (char *)&opt, sizeof(opt))) {
    perror("setsockopt");
    status = MPI_ERR_OTHER;
    goto close;
  ...
  if (bind(comm->socket_info.server_fd, (struct sockaddr *)&address,
           sizeof(address)) < 0) {
    char bind_msg[1024] = {'\0'};
    snprintf(bind_msg, 1023,
             "function %s, file %s, line %d, rank %d, port %d, bind error",
  ...
  for (i = rank - 1; i >= 0; i--) {
    // Here "address" is a wildcard--meaning accept a connection from any
    // address. However, since connect/accept is blocking and we "connect" to
    // the next higher/lower rank, we fill out client_fds in rank order e.g.
    // ranks are 0 1 2 0 connects to 1 and blocks 1 connects to 2 and blocks 2
    // accepts 1, unblocking 1. 1 is in rank order in 2's client_fds 2 accepts
    // 0, 0 is in rank order in 2's client_fds 1 accepts 0, 0 is in rank order
    // in 1's client_fds
    // TODO: is this explanation necessary?
    comm->socket_info.client_fds[i] =
        accept(comm->socket_info.server_fd, (struct sockaddr *)&address,
               (socklen_t *)&addrlen);
  }
...
static int init_clients(nanompi_communicator_t *comm) {
  ...
  // Make a client socket for every rank greater than mine. Last rank just
  // returns without doing anything
  for (i = rank + 1; i < size; i++) {
    // Get ip of hostname (resolve dns)
    if (err = getaddrinfo(grp_proc_pointers[i]->hostname, NULL, &hints, &res) !=
              0) {
      printf("getaddrinfo failed: %s\n", gai_strerror(err));
      goto free;
    }

    if ((comm->socket_info.client_fds[i] = socket(AF_INET, SOCK_STREAM, 0)) <
        0) {
    ...
    // This client may have reached here before the server called accept(), so
    // just keep trying
    while (
        connect(comm->socket_info.client_fds[i], res->ai_addr, res->ai_addrlen))
      ;
...
int nanompi_socket_send(const void *buffer, size_t msg_size, int to_rank,
  ...
  while (sent_bytes != msg_size) {
    sent_bytes += send(comm->socket_info.client_fds[to_rank],
                       buffer + sent_bytes, msg_size - sent_bytes, 0);
  }
