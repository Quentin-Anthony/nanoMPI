...
void *execute_command_on_host(void *args) {
  ...
  snprintf(
      ssh_command, sizeof(ssh_command),
      "ssh %s 'export NANOMPI_WORLD_RANK=%d ; export NANOMPI_WORLD_SIZE=%d ; "
      "export NANOMPI_HOSTFILE=%s ; export LD_LIBRARY_PATH=%s ; cd %s ; %s'",
      exe_args->hostname, exe_args->rank, exe_args->size,
      exe_args->hostfile_path, getenv("LD_LIBRARY_PATH"), getenv("PWD"),
      exe_args->cmd);
...
int main(int argc, char *argv[]) {
  ...
  while (fgets(line, sizeof(line), file)) {
    ...
    if (pthread_create(&threads[rank], NULL, execute_command_on_host,
                       &args[rank]) != 0) {
