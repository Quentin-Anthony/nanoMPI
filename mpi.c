...
int MPI_Init(int *argc, char ***argv) {
  ...
  status =
      nanompi_init_comm(&nanompi_comm_world, my_rank, world_size, hostfile);
  if (status) {
...
// TODO: tags, generate a message envelope with match info so the recv side can
// match
int MPI_Send(const void *buf, int count, MPI_Datatype datatype, int dest,
  ...
  nanompi_message_envelope envelope = {.sizeof_buffer = msg_size, .tag = tag};
...
// TODO: tags, MPI_ANY_SOURCE, matching with a variable (but lesser than) count
// that the user passed, etc.
